...
find "$SRC/jsonpickle/fuzzing" -name 'fuzz_*.py' -print0 | while IFS= read -r -d '' fuzz_harness; do
  ...
  # Strip the `.py` extension from the filename and replace it with `_seed_corpus.zip`.
  seed_corpus_filename="$(basename "$fuzz_harness" .py)_seed_corpus.zip"
  if [[ ! -f "$OUT/$seed_corpus_filename" && -s "$SEED_DATA_DIR/__default_corpus.zip" ]]; then
     # If a seed corpus does not already exist for this fuzz harness, and a default is available, copy it.
     printf '[%s] Copying %s to %s\n' "$(date '+%Y-%m-%d %H:%M:%S')" "$SEED_DATA_DIR/__default_corpus.zip" "$OUT/$seed_corpus_filename"
     cp "$SEED_DATA_DIR/__default_corpus.zip" "$OUT/$seed_corpus_filename"
  fi
